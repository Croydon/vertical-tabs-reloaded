...
function dragover(event)
{
	...
	//var workaroundVar = this;
	
	//this.tabBrowser._setEffectAllowedForDataTransfer = function(aEvent) {
		//var mungeEvent = aEvent;
		//event.__defineGetter__("screenX", function() { return -1000; });
		//return workaroundVar._setEffectAllowedForDataTransfer(mungeEvent);
	//};
	...
	if (effects == "" || effects == "none") {
		ind.collapsed = true;
		console.log("blub funcon ending");
		return;
	}
	console.log("this should not get outputted");
...
function dragend(event)
{
	...
	if (this.tabbrowser.tabs.length == 1) {
		...
		this.tabbrowser.replaceTabWithWindow(draggedTab, { screenX: left, screenY: top, });
	}
	event.stopPropagation();
}

function _getDropIndex(event)
{
