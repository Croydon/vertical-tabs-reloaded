...
VerticalTabsReloaded.prototype = {
    ...
    webext_sendMsg: function(message)
    {
        // Send message to WebExtension
        this.webextPort.postMessage(message);
    },

    webext_sendChangedSetting: function (settingName, newValue)
    {
        // Changed addon preferences, redirect through WebExt to save eventually
        this.webext_sendMsg({
            type: "settings.post-to-sdk",
            name: settingName,
            value: newValue
        });
    },
	...
	rearrangeUI: function() {
        ...
        if(this.preferences("tabtoolbarPosition") == "hide") 
        {
            toolbar.style.display = "none";
        }
        
		if (this.preferences("tabtoolbarPosition") == "bottom") {	
	...
	onPreferenceChange: function(prefName, newValue) 
    {
		switch (prefName) 
        {
			...
			case "tabtoolbarPosition":
                this["tabtoolbarPosition"] = newValue;
	...
	*/
	onSizeModeChange: function() {
		...
		let hideOk = this.preferences("hideInFullscreen");
